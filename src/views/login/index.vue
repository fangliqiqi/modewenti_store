...
export default {
  ...
  data() {
    ...
    const validateCode = (rule, value, callback) => {
      ...
      } else if (this.loginForm.code.length == this.loginForm.key.length) {
        if (this.loginForm.code.toUpperCase()!=this.loginForm.key.toUpperCase()) {
          // this.getCode()
    ...
    return {
      captchatImg: '',
      swiperList: [],
      loginLogo: require("@/assets/imgs/live-logo.png"),
      backgroundImages: '',
      ...
      loginRules: {
        ...
        code: [{ required: true, message: '请输入正确的验证码', trigger: 'blur',validator: validateCode }]
      },
  ...
  methods: {
    ...
    handleLogin() {
      ...
      this.$refs.loginForm.validate(valid => {
        if (valid) {
          ...
          this.$store.dispatch('user/login', {account:form.username,password:encrypt(form.password),isSave:form.isSave})
            .then(() => {
              this.$router.push({ path: this.redirect || '/', query: this.otherQuery })
              // getStoreStaff()
              this.loading = false
    ...
    getCaptcha() {
      captchaApi().then( res  => {
        this.captchatImg = res.base64Image;
        this.loginForm.key = res.verifyCode;
      }).catch(({ message }) => {
