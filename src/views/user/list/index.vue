    ...
    <el-card class="box-card">
            ...
            <el-dropdown trigger="click">
              ...
              <el-dropdown-menu slot="dropdown">
                ...
                <el-dropdown-item @click.native="clearSpread(scope.row)"
                                  v-if="scope.row.spreadUid && scope.row.spreadUid>0">清除上级推广人
                </el-dropdown-item>
              </el-dropdown-menu>
            </el-dropdown>
          </template>
      ...
      <div class="block">
        <el-pagination
          ...
          :page-size="userFrom.pageSize"
          :current-page="userFrom.pageNo"
    ...
    <el-dialog
      ...
      <el-form :model="PointValidateForm" ref="PointValidateForm" label-width="100px" class="demo-dynamic"
        ...
        <el-form-item
          ...
          required
        >
          <el-input-number type="text" step-strictly v-model="PointValidateForm.integralValue"
  ...
  methods: {
    ...
    submitForm(formName) {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          this.loading = true
          this.batchName === 'group' ? groupPiApi({
    ...
    // 分组列表
    groupLists() {
      groupListApi({pageNo: 1, pageSize: 9999}).then(async res => {
        this.groupList = res.rows
      })
    },
    //标签列表
    getTagList() {
      tagListApi({pageNo: 1, pageSize: 9999}).then(res => {
        console.info("标签列表")
        console.info(res)
        this.labelLists = res.rows
      })
    },
    // 等级列表
    levelLists() {
      levelListApi({pageNo: 1, pageSize: 9999}).then(async res => {
        this.levelList = res.rows
        localStorage.setItem('levelKey', JSON.stringify(res.rows))
      })
    },
    // 列表
    getList(num) {
      this.listLoading = true
      this.userFrom.pageNo = num ? num : this.userFrom.pageNo;
