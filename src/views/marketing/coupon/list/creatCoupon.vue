...
<!--            <el-radio :label="1">通用券</el-radio>-->
        ...
        <el-form-item label="选择品类：" prop="primaryKey" v-if="ruleForm.useType === 3">
          <el-cascader v-model="ruleForm.primaryKey" :options="merCateList" :props="props2" clearable class="selWidth"
                       :show-all-levels="false"/>
        </el-form-item>
        <el-form-item label="商品：" v-if="ruleForm.useType === 2" prop="image">
  ...
  methods: {
    ...
    changeGood() {
      ...
      this.$modalGoodList(function (row) {
        _this.ruleForm.image = row.image
    ...
    submitForm(formName) {
      ...
      if (this.ruleForm.useType === 2) this.ruleForm.primaryKey = this.productId


      // if (this.ruleForm.useType === 2) this.ruleForm.primaryKey = this.ruleForm.checked.map(item => {
      //   return item.id
      // }).join(',')
