        ...
        <el-row v-show="currentTab === 0" :gutter="24">
          ...
          <el-col v-bind="grid2">
            <el-form-item label="商品名称：" prop="storeName">
              <el-input v-model="formValidate.storeName" placeholder="请输入商品名称" maxlength="128"/>
            </el-form-item>
          </el-col>
          <el-col v-bind="grid2">
              ...
              <el-table :data="oneFormBatch" border class="tabNumWidth" size="mini">
                ...
                <template v-if="formValidate.isSub">
                  ...
                  <el-table-column align="center" label="二级返佣(元)" min-width="120">
                    <template slot-scope="scope">
                      <el-input v-model.number="scope.row.brokerageTwo" type="number" :min="0" class="priceBox" />
                    </template>
                  </el-table-column>
                </template>
    ...
    methods: {
      ...
      getCategorySelect() {
        categoryApi({ status: -1, type: 1 }).then(res => {
          this.merCateList = this.filerMerCateList(this.$treeFormat(res,'child'))
        })
      },
      ...
      handleSubmit (name) {
        ...
        this.$refs[name].validate((valid) => {
          if (valid) {
            this.fullscreenLoading = true
            this.formValidate.merId = this.$store.getters.shopId;
