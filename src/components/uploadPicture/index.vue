          ...
          <div class="pictrueList acea-row">
            ...
            <div class="conters">
              <div
                ...
                class="gridPic"
              >
                <img v-lazy="item.sattDir?item.imgUrl:localImg"  :class="item.isSelect ? 'on': '' " @click="changImage(item, index, pictrueList.list)" />
...
export default {
  ...
  methods: {
    ...
    getFileList() {
      let path = '';
      try {
        path =  process.env.VUE_APP_IMAGE_API
      }catch (e) {
        console.info(e)
      }
      if ( this.tableData.pid === 10000) this.tableData.pid = 0
      fileListApi(this.tableData).then(async res => {
        this.pictrueList.list = res.rows.map(item=>{
            item.imgUrl = path + item.sattDir;
          return item;
        })
        if(this.tableData.pageNo === 1 && this.pictrueList.list.length > 0) this.pictrueList.list[0].localImg = this.localImg
        if (this.pictrueList.list.length) {
          this.isShowPic = false
        } else {
          this.isShowPic = true
        }
        this.pictrueList.total = res.totalRows
      })
    },
    pageChange(page) {
    ...
    checkPics() {
        ...
        if (this.$route && this.$route.query.field === 'dialog') {
          ...
          for (let i = 0; i < this.checkPicList.length; i++) {
            str += '<img src="' + this.checkPicList[i].imgUrl + '">'
          }
          console.info(str)
          /* eslint-disable */
