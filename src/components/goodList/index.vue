    ...
    >
      <el-table-column
        ...
        <template slot="header" slot-scope="scope">
          <el-checkbox slot="reference" :value="isChecked && checkedPage.indexOf(tableFrom.pageNo) > -1" @change="changeType" />
        </template>
...
export default {
  ...
  methods: {
    ...
    changeType(v) {
      this.isChecked = v
        const index = this.checkedPage.indexOf(this.tableFrom.pageNo)
        this.isIndex = index
        this.checkedPage.push(this.tableFrom.pageNo)
        if (index > -1) {
    ...
    getList(num) {
      ...
      this.tableFrom.pageNo = num ? num : this.tableFrom.pageNo;
      productLstApi(this.tableFrom).then(res => {

        this.tableData.data = res.rows
        this.tableData.total = res.totalPage
